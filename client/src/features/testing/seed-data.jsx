 ...
 */
  ...
  const editObjForDb = (obj) => {
    ...
    if (Object.hasOwn(obj, "addDate"))
      obj.addDate = new Date(obj.addDate * 1000).toISOString();
    if (Object.hasOwn(obj, "lastModified"))
      obj.lastModified = new Date(obj.lastModified * 1000).toISOString();
    return obj;
  };
  ...
  const addData = (arr, depth = 0) => {
    arr.forEach(async (element) => {
      if (element.type === "link")
        await addLink(element, parentIdAtDepth.get(depth));
      if (element.type === "folder") {
        ...
        if (Object.hasOwn(element, "children"))
          addData(element.children, incrementDepth);
      }
    });
  };
